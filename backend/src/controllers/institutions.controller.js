  ...
  try {
    ...
    res.json({
      data: [
        {
          ...data,
          prefijo: institutionInitials(data.nombre),
          alumnos: await studentCount(data.id),
        },
      ],
    });
  } catch (e) {
...
export async function createInstitution(req, res, next) {
  try {
    ...
    res
      .status(201)
      .json({ data: { ...data, prefijo: institutionInitials(data.nombre), alumnos: 0 } });
  } catch (e) {
    next(e);
  }
}
  ...
  try {
    ...
    const [{ count: alumnosCount }, { count: usuariosCount }] = await Promise.all([
      supabase
        .from('alumnos')
        .select('*', { count: 'exact', head: true })
        .eq('institucion_id', req.params.id),
      supabase
        .from('usuarios')
        .select('*', { count: 'exact', head: true })
        .eq('institucion_id', req.params.id),
